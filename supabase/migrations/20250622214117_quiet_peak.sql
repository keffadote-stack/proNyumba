/*
  # Complete Database Schema for NyumbaTZ Rental Platform
  
  This migration creates the complete database schema for the rental platform
  including all tables, policies, functions, and initial data setup.
...
CREATE TABLE IF NOT EXISTS property_views (
  ...
  UNIQUE(property_id, viewer_id, date_trunc('hour', viewed_at))
);
...
BEGIN
  ...
  ON CONFLICT (property_id, viewer_id, date_trunc('hour', viewed_at)) DO NOTHING;
...
BEGIN
    ...
    AND routine_name IN ('handle_new_user', 'update_updated_at_column', 'update_properties_search_vector', 'increment_property_views', 'increment_inquiry_count');
